        ...
        children: [
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                const Text('后端地址（例如：http://192.168.x.x:8090）'),
                  ...
                  children: [
                    ElevatedButton(
                      onPressed: () async {
                        ...
                        if (url.isEmpty) {
                          Fluttertoast.showToast(msg: '请输入后端地址');
                          return;
                        }
                        try {
                          await ApiClient().configureBaseUrl(url);
                          Fluttertoast.showToast(msg: '已保存后端地址');
                        } catch (e) {
                    ...
                    OutlinedButton(
                      onPressed: () async {
                        setState(() => _apiStatus = '测试中...');
                        try {
                          final resp = await ApiClient().dio.get('/health');
                          setState(() => _apiStatus = '连接正常：${resp.data['status']}');
                          Fluttertoast.showToast(msg: '连接正常');
                        } catch (e) {
                          setState(() => _apiStatus = '连接失败：$e');
